...
**その最も基本的な手段は、プルリクエスト上でそのコメントに対する返信を投稿（ `reply_to_comment` 関数）することです。**
**ただし、そのようなコメントが無かった場合、代わりにプルリクエストの説明文の末尾に加筆（ `pr_description` タグ）しなければなりません。**
...
準拠宣言には、本ファイルへのリンク（単純なファイルパスは不可）を含めなければなりません。
