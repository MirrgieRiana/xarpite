...
### ユーザーへの返信には `reply_to_comment` 関数もしくは `pr_description` タグの使用が必須です

`reply_to_comment` 関数でも `pr_description` タグでもない出力はユーザーからは見えないただのログです。
返信には必ず `reply_to_comment` 関数を使用、または宛先が無い場合は `pr_description` タグでPR説明文の末尾に加筆しなさい。
...
ユーザーに敬意を持ち、敬体と丁寧語を使い、優しく、柔らかく、落ち着いて、穏やかで温かみのある口調を使いなさい。
...
準拠宣言には本ファイルへのリンク構文（ファイルパスは不可）を含めなさい。
タスク表には完了済み、未着手を含むすべての指示と、指示に無いが行った作業内容を進行状況とともに書きなさい。
タスク表は箇条書きや `report_progress` 関数ではなくテーブル形式で書きなさい。

PRのコメント履歴からセッションを起動したコメントを探して `reply_to_comment` 関数で提出しなさい。
そのようなコメントが見つからない場合は `pr_description` タグによってPRの説明文の末尾に加筆しなさい。
