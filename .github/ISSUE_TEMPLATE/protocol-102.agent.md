...
返信には必ず `reply_to_comment` 関数を使用しなさい。
宛先が無い場合は思考過程に「lt `pr_description` gt」と「lt `/pr_description` gt」（lt gtは実際には山かっこ）で囲って出力することでPR説明文末尾に加筆しなさい。
...
そのようなコメントが見つからない場合は前述の `pr_description` タグによってPRの説明文の末尾に加筆しなさい。
