...
class CliTest {
    ...
    @Test
    fun useIncWithFileUrl() = runTest {
        val context = TestIoContext()
        if (getFileSystem().isFailure) return@runTest
        val fileSystem = getFileSystem().getOrThrow()

        // INCディレクトリにモジュールを配置
        val incDir = "build/test/use.inc.file.url.tmp".toPath()
        fileSystem.createDirectories(incDir)
        val moduleFile = incDir.resolve("strawberry.xa1")
        fileSystem.write(moduleFile) { writeUtf8("\"Strawberry\"") }

        // 絶対パスを取得
        val absolutePath = incDir.normalized().toString()
        
        // file:// URLを構築
        val fileUrl = "file://$absolutePath"
        
        // file:// URL形式のINCパスを追加してモジュールをロード
        val result = cliEval(context, """
            INC::push("$fileUrl")
            USE("strawberry")
        """.trimIndent()).toFluoriteString(null).value
    ...
    @Test
    fun useIncWithFileUrlWithExtension() = runTest {
        val context = TestIoContext()
        if (getFileSystem().isFailure) return@runTest
        val fileSystem = getFileSystem().getOrThrow()

        // INCディレクトリにモジュールを配置
        val incDir = "build/test/use.inc.file.url.ext.tmp".toPath()
        fileSystem.createDirectories(incDir)
        val moduleFile = incDir.resolve("blueberry.xa1")
        fileSystem.write(moduleFile) { writeUtf8("\"Blueberry\"") }
    ...
    @Test
    fun useIncWithFileUrlWithSubdirectory() = runTest {
        val context = TestIoContext()
        if (getFileSystem().isFailure) return@runTest
        val fileSystem = getFileSystem().getOrThrow()

        // INCディレクトリにサブディレクトリを含むモジュールを配置
        val incDir = "build/test/use.inc.file.url.sub.tmp".toPath()
        val subDir = incDir.resolve("fruit")
        fileSystem.createDirectories(subDir)
        val moduleFile = subDir.resolve("raspberry.xa1")
        fileSystem.write(moduleFile) { writeUtf8("\"Raspberry\"") }
        ...
        // サブディレクトリを含むfile:// URL相対パスでモジュールをロード
        val result = cliEval(context, """
            INC::push("$fileUrl")
            USE("fruit/raspberry")
        """.trimIndent()).toFluoriteString(null).value
