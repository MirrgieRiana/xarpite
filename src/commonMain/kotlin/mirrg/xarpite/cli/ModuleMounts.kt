...
fun createModuleMounts(filePath: String, mountsFactory: (String) -> List<Map<String, FluoriteValue>>): List<Map<String, FluoriteValue>> {
    return mapOf(
        "USE" to run {
            val moduleCache = mutableMapOf<okio.Path, FluoriteValue>()
            val baseDir by lazy {
                val parent = filePath.toPath().parent
                parent?.normalized() ?: throw FluoriteException("Parent directory is required".toFluoriteString())
            }
