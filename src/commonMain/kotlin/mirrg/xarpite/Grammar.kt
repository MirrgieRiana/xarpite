...
class XarpiteGrammar(val location: String) {
    ...
    val float: Parser<Node> = +Regex("""[0-9](?:_*[0-9])*\.[0-9](?:_*[0-9])*""") map { FloatNode(it.value) }

    val integer: Parser<Node> = +Regex("[0-9](?:_*[0-9])*") map { IntegerNode(it.value) }

    val hexadecimal: Parser<Node> = +Regex("H#[0-9a-fA-F](?:_*[0-9a-fA-F])*") map { HexadecimalNode(it.value.drop(2)) }
