...
fun createDataConversionMounts(): List<Map<String, FluoriteValue>> {
    return mapOf(
        ...
        "BASE64" to FluoriteFunction { arguments ->
            fun usage(): Nothing = usage("BASE64(blobLike: BLOB_LIKE | STREAM<BLOB_LIKE>): STRING | STREAM<STRING>")
            if (arguments.size != 1) usage()
            val value = arguments[0]
                ...
                FluoriteStream {
                    ...
                    value.collect { item ->
                        ...
                        while (buffer.size >= 3) {
                            ...
                            val encoded = Base64.Mime.encode(chunk)
                    ...
                    if (buffer.isNotEmpty()) {
                        val encoded = Base64.Mime.encode(buffer.toByteArray())
                        for (char in encoded) {
                            if (lineLength >= 76) {
                                resultBuilder.append('\n')
                                lineLength = 0
                    ...
                    emit(resultBuilder.toString().toFluoriteString())
                }
            } else {
                // 単一の値の場合
                val bytes = value.toByteArrayAsBlobLike()
                Base64.Mime.encode(bytes).toFluoriteString()
            }
        },
        "BASE64D" to FluoriteFunction { arguments ->
            ...
            if (value is FluoriteStream) {
                ...
                FluoriteStream {
                    ...
                    value.collect { item ->
                        ...
                        while (buffer.length >= 4) {
                            ...
                            buffer.delete(0, 4)
                            val decoded = Base64.Mime.decode(chunk)
                    ...
                    // 残りの文字を処理（パディングを含む）
                    if (buffer.isNotEmpty()) {
                        val decoded = Base64.Mime.decode(buffer.toString())
                        resultBuffer.addAll(decoded.toList())
                    }

                    emit(resultBuffer.toByteArray().asFluoriteBlob())
                }
            } else {
                // 単一の値の場合
                ...
                Base64.Mime.decode(cleaned).asFluoriteBlob()
            }
        },
