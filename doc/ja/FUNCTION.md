...
前置 `\` 演算子は、 `_ -> body` という1引数のラムダ式を作る糖衣構文です。

`_` という名前の引数を持つラムダ式を簡潔に記述できます。
...
この関数生成演算子は、 `FILTER` 関数や `SORT` 関数などの高階関数と組み合わせると便利です。

例えば、配列の要素をフィルタリングしたり、特定のプロパティでソートしたりする際に簡潔に記述できます。

```shell
$ xa 'f := \(_ * 2); f(5)'
# 10

$ xa '[1, 2, 3, 4, 5] | FILTER(\(_ > 3))'
# 4
# 5

$ xa '[{name: "Bob"; age: 30}, {name: "Alice"; age: 25}] | SORT[by: \_.name]'
# {name:Alice;age:25}
# {name:Bob;age:30}
```
