...
The basic specifications are the same as the `replace` method.

Unlike the `replace` method, the `replaceFirst` method always replaces only the first occurrence.
