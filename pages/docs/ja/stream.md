...
Xarpiteの型文化において、任意の型`T`は`STREAM<T>`に暗黙にキャスト可能であると解釈されます。
...
`end`が`start`よりも小さい場合、カウントダウンを行います。
...
半開区間演算子は閉区間演算子とは異なり、`end`が`start`よりも小さい場合、空のストリームを生成します。
...
`FIRST`などによる先読みの中断で途中で打ち切られたイテレーションも、1回のイテレーションとして数えられます。
...
1要素のストリームや非ストリームを渡した場合は、その値がそのまま返されます。
