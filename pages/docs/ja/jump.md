...
`ERROR` 型の値をスローした場合は、その値そのものではなく、それが表しているネイティブエラーが再スローされます。
...
キャッチ演算子は、スロー演算子 `!!` によってスローされた値（ `FluoriteException` ）だけでなく、ホスト言語に由来するネイティブエラーも捕捉します。
...
`ERROR` は、ホスト言語に由来するネイティブエラーを表すクラスです。

`値 ?= ERROR` により、値がネイティブエラーであるかどうかを判定できます。
