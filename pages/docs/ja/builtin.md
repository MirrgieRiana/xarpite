...
ストリームの要素を指定されたキーでグループ化し、エントリー配列のストリームとして返します。

エントリー配列は、最初にそのキーが現れた順序になります。

`GROUP` は、2種類の呼び出し方があります。

### 要素そのものをキーとしてグループ化

`<T> GROUP(stream: T,): [T; [T,]],`

1引数で呼び出した場合、ストリームの要素そのものをキーとしてグループ化します。
...
第1引数が `by` パラメータである場合、第2引数の各要素に対して `key_getter` 関数を適用し、その結果をキーとしてグループ化します。
