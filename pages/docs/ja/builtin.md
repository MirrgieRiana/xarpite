...
初期化関数を受け取り、最初の呼び出し時のみその関数を実行して結果をキャッシュする関数を返します。

2回目以降の呼び出しでは、キャッシュされた値を返します。

初期化関数がストリームを返した場合、ストリームは解決されてからキャッシュされます。
...
委譲変数と組み合わせることで、最初のアクセス時の値を保持する変数を作成できます。
