...
インデックスは0から始まります。負のインデックスは末尾からのインデックスを表します（-1は最後の要素、-2は最後から2番目の要素）。該当するインデックスが存在しない場合は `NULL` を返します。

`indices` が非ストリームの場合、戻り値も非ストリームになります。`indices` がストリームの場合、戻り値はストリームになります。
