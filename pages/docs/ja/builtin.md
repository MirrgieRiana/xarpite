...
## `INTERCALATE` 配列のストリームをストリームに連結

`<T> INTERCALATE(separator: ARRAY<T>; arrays: STREAM<ARRAY<T>>): STREAM<T>`

第2引数の配列のストリームの各配列を第1引数のセパレータ配列で連結したストリームを返します。

各配列の間にセパレータ配列が挿入されますが、先頭や末尾にはセパレータは付きません。

結果は逐次的に出力されるストリームであり、配列に固められることはありません。
...
`INTERCALATE` は `JOIN` の配列版であり、文字列ではなく配列を扱います。結果はストリームとして逐次的に返されます。
