...
注意: ストリームリテラル `(1, 2, 3)` に対して直接 `::LET` を呼び出すことはできません。これは、`::` 演算子がストリームを展開してから各要素にメソッドを適用するためです。
...
注意: ストリームリテラル `(1, 2, 3)` に対して直接 `::ALSO` を呼び出すことはできません。これは、`::` 演算子がストリームを展開してから各要素にメソッドを適用するためです。
