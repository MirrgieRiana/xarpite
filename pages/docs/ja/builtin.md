...
`::` 演算子は、ストリームリテラル `(1, 2, 3)` に対して使用すると、ストリームを展開してから各要素にメソッドを適用します。

このため、ストリームそのものに対して `::LET` を呼び出すには、関数名を丸括弧で囲む構文 `(LET)` を使用します。
...
この構文は、メソッド解決ではなく関数解決を先に行い、それをメソッドのように呼び出します。
...
ストリームそのものに対して `::ALSO` を呼び出すには、関数名を丸括弧で囲む構文 `(ALSO)` を使用します。
