...
文字列を改行で分割し、各行をストリームとして返します。

改行文字（LF、CR、CRLF）はすべて認識され、結果の各行からは改行文字が除去されます。

最後の改行は削除され、文字列の末尾に改行があるかどうかの情報は失われます。

改行文字が何だったか（LF、CR、CRLF）の情報も失われます。
...
改行のみの文字列の場合も空のストリームを返します（末尾の改行が削除されるため）。
