...
## `SEMVER`セマンティックバージョン文字列を比較可能な値に変換

`SEMVER(version: STRING): OBJECT`

`version`をセマンティックバージョンとして解釈し、宇宙船演算子`<=>`や比較演算子で比較できる値を返します。

戻り値の内部構造は隠蔽されており、バージョンの比較以外の用途を意図していません。

`version`をセマンティックバージョンとして解釈できない場合、エラーが発生します。

数値コアである`major.minor.patch`は、辞書順ではなく各要素を数値として比較します。
...
`-`に続くプレリリースを持つバージョンは、同じ数値コアのプレリリースを持たないバージョンよりも小さいと判定されます。
プレリリースの識別子同士は、`.`で区切られた要素を左から順に比較します。
数値のみの識別子は数値として、それ以外の識別子は辞書順で比較し、数値のみの識別子はそれ以外の識別子よりも小さいと判定されます。
先行する識別子がすべて等しい場合、識別子の個数が多いバージョンの方が大きいと判定されます。
