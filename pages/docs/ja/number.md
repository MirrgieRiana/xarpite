...
数値を表す型には32ビット整数 `INT`、任意精度整数 `BIG`、64ビット浮動小数点数 `DOUBLE` があります。
...
`INT` の範囲に収まらない大きさの整数は、任意精度整数型 `BIG` で扱われます。

### INTの範囲を超える整数の数値化

`INT` の範囲に収まらない整数を表す文字列を数値化すると、精度を失わずに `BIG` になります。
...
これにより、巨大な整数を含むJSONを、精度を保ったままデコードできます。
