...
これらのメソッドをオブジェクトに定義することで、インクリメント・デクリメントの動作をカスタマイズできます。

オーバーライドされたメソッドは、オブジェクト自身をミューテーション（値の変更）することを前提としています。
...
オーバーライドされたメソッドが存在する場合、変数への代入は行われません。メソッド内でオブジェクトをミューテーションする必要があります。
