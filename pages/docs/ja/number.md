...
先頭に `_` を置くことはできません。先頭に `_` を置いた場合、識別子として扱われます。
...
整数リテラルと同様に、16進数の数字の後に `_` が任意個続いてもよく、`_` は無視されます。
...
整数リテラルと同様に、数字の後に `_` が任意個続いてもよく、`_` は無視されます。

整数部と小数部の両方で `_` を使用できます。
