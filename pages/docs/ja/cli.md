...
`IN` を一度でも使用した場合、 `INB` および `INC` を使用することはできません。

### `INC`: コンソールから文字列を1文字ずつ読み取る

`INC: STREAM<STRING>`

標準入力から文字列を1文字ずつ読み取るストリームです。
...
コンソールから1文字だけ読み取るには、 `FIRST` 関数を使用します。
...
`INC` を一度でも使用した場合、 `IN` および `INB` を使用することはできません。
...
`INB` を一度でも使用した場合、 `IN` および `INC` を使用することはできません。
...
ストリームが指定された場合、各要素を文字列化して1文字ずつ出力します。
...
長さが2以上の文字列が指定された場合、その文字列全体を1度に出力します。
