...
コードがファイルから読み込まれていない場合（ `-e` オプションによる直接評価など）、 `NULL` が返ります。
...
`-f` オプションでファイルを指定した場合、そのファイルの絶対パスが `LOCATION` に格納されます。

---

`USE` 関数によって読み込まれたモジュール内では、そのモジュールファイル自身の絶対パスが `LOCATION` に格納されます。

### `LOCATION_DIR`: 実行中のファイルの親ディレクトリを取得

`LOCATION_DIR: STRING | NULL`

現在実行中のXarpiteスクリプトファイルが存在するディレクトリの絶対パスです。

コードがファイルから読み込まれていない場合でも、スクリプトディレクトリ（PWDと同等）の絶対パスが返されます。

```shell
$ cd /tmp && xa -e 'LOCATION_DIR'
# /tmp
```

### `LOCATION_FILE`: 実行中のファイル名を取得

`LOCATION_FILE: STRING | NULL`

現在実行中のXarpiteスクリプトファイルのファイル名（ディレクトリパスを含まない）です。

コードがファイルから読み込まれていない場合、 `NULL` が返ります。
