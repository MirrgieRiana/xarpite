...
検索パスには通常のファイルパスの他に、 `http://` 、 `https://` などのURL形式も指定できますが、現在の実装ではモジュールの検索パスとして解決されるのみで、実際にネットワーク経由でモジュールを取得する機能は未実装です。

URL形式で指定された場合、そのURLの子パスを検索パスとして生成しますが、実際のモジュールロードはローカルファイルシステムからのみ行われます。
...
URL形式の指定例（ネットワークロードは未実装）：

```
INC::push("https://example.com/xarpite/modules")
USE("com.example:mylib:1.0.0")
# https://example.com/xarpite/modules/com/example/mylib/1.0.0/mylib-1.0.0.xa1 が検索パスとして解決される
```
