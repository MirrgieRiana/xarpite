...
`reference` には、絶対ローカルファイルパス、URL、相対パス、Maven座標の指定方法があります。
...
`reference` は検索場所を与える `INC` ビルトイン定数に基いて実際のロケーションに解決されます。
...
`-e` コマンドラインオプションによって起動されたコンテキストでは、ロケーションは `PWD` 直下の `-` という名前のファイルとして扱われています。
