...
### `BASH`: Bashシェルスクリプトを実行

`BASH(script: STRING): STRING`

Bashシェルスクリプトを実行し、その標準出力を文字列として返します。

`script` にはBashで実行するシェルスクリプトを指定します。

戻り値は実行結果の標準出力をUTF-8としてデコードした文字列です。

末尾に改行 `\n` が存在する場合、それは取り除かれます。

これはBashの `"$()"` コマンド置換と同じ動作です。
...
この関数は内部的に `bash -c <script>` というコマンドを実行します。

環境変数の指定やその他のBashオプションはサポートされません。

それらの機能が必要な場合は `EXEC` 関数を使用してください。
