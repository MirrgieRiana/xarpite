...
`reference` が `.` または `..` の階層で始まらない相対パスである場合、対応するモジュールファイルを `INC` に登録されたディレクトリやURLから検索します。

`INC` 配列内で先頭に近いパスに属するモジュールファイルが優先されます。

ローカルパスのエントリはURL形式のエントリよりも優先的に検索されます。

ディレクトリの区切り文字は、それが実行されるOSに関わらず `/` を使用することができます。

パスの末尾の `.xa1` もしくは `/main.xa1` は省略可能です。ただし、URL形式のエントリでは `/main.xa1` へのフォールバックは行われません。
...
`reference` がMaven座標形式である場合、対応するモジュールファイルを `INC` に登録されたディレクトリやURLから検索します。
