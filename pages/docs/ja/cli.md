...
`-q` オプションを忘れると、バイト列の最初に「 `NULL` 改行」などのゴミが付きます。
...
`OUTB` を使用する際には、 `OUTB` がNULLを返すこと、そして `-q` オプションを使用することを忘れないようにしてください。
...
この関数は `OUT` 関数と同様に動作しますが、標準出力ではなく標準エラー出力に書き込みます。

```shell
$ xa -q 'ERR("エラーメッセージ")' 2>&1
# エラーメッセージ
```
...
この関数は `OUTB` 関数と同様に動作しますが、標準出力ではなく標準エラー出力に書き込みます。
...
```shell
$ xa -q 'ERRB(BLOB.of([65, 66, 67, 10]), BLOB.of([68, 69, 70, 10]))' 2>&1
# ABC
# DEF
```
...
それ以外の部分は概ね `ERR` 関数および `OUTB` 関数の仕様に準じます。

---

`ERRB` を使用する際には、 `ERRB` がNULLを返すこと、そして `-q` オプションを使用することを忘れないようにしてください。
