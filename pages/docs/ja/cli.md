...
例えば `256M` は256メビバイトを表します。

---

単位を表す接尾辞は必須であり、これを省略した場合や不正な値を指定した場合はエラーとなります。

---

この指定はすべてのエンジンに適用されます。

---

深い再帰など、多くのスタックを消費する処理によるスタックオーバーフローは、この上限を引き上げることで緩和できます。

ただし、これはスタックオーバーフローを完全に防ぐものではなく、指定した上限を超える深さの処理は依然としてスタックオーバーフローを引き起こします。
