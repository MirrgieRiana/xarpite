...
### `XARPITE_STACK_SIZE_KB`: スタックサイズの上限を指定する環境変数

`XARPITE_STACK_SIZE_KB` 環境変数はXarpiteのスタックサイズの上限を指定します。

値はキビバイト（1024バイト）単位で指定し、 `ulimit -s` にそのまま渡されます。

この環境変数が指定されていない場合、デフォルトで `262144` （256MiB）が使用されます。

---

ネイティブエンジンでは、可変長の繰り返しを含む複雑な正規表現のマッチングなどにおいて、繰り返し回数に比例した深さの関数呼び出しが行われ、スタックを使い切るとプロセスがクラッシュ（SIGSEGV）することがあります。

この環境変数で上限を引き上げることで、より深い呼び出しを許容できます。

---

この環境変数はLinux版のランチャースクリプトを経由してネイティブエンジンを起動した場合にのみ効果があります。

JVMエンジンやNode.jsエンジン、およびランチャースクリプトを経由しない起動では無視されます。
