...
宇宙船演算子のオーバーライドが行われると、各種比較演算子の動作も変わります。
...
厳密等価演算子`left === right`は、両辺の値が厳密に等しいか否かを返します。

両辺の型が異なる場合は、常に厳密に等しくはないと判定します。

両辺の型が同じ場合は、型に応じて、次の表のように、厳密に等しいかを判定します。
...
| `NULL` | NULL | 内容の一致 |
| `REGEX` | 正規表現 | 内容の一致 |
| `OBJECT` | オブジェクト | インスタンスの同一性 |
| `ARRAY` | 配列 | インスタンスの同一性 |
| `BLOB` | BLOB | インスタンスの同一性 |
| `STREAM` | ストリーム | インスタンスの同一性 |
| `PROMISE` | Promise | インスタンスの同一性 |
| `FUNCTION` | 関数 | インスタンスの同一性 |
| `ERROR` | エラー | インスタンスの同一性 |
...
厳密不等価演算子`left !== right`は、厳密等価演算子`left === right`の否定版です。
