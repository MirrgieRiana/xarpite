...
厳密等価演算子 `left === right` は、両辺が厳密に等しいか否かを返します。

両辺の型が異なる場合は、常に等しくないと判定します。
両辺の型が同じ場合、数値や文字列のような不変な型は値の内容で、オブジェクトや配列のような可変な型はインスタンスの同一性で比較します。
...
厳密不等価演算子 `left !== right` は、両辺が厳密に等しくないか否かを返します。
