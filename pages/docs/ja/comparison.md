...
論理値同士を比較した場合は、`FALSE`が`TRUE`より小さいものとして判定されます。
...
比較できるのは、数値同士、文字列同士、論理値同士といった、同じカテゴリに属する値同士に限られます。カテゴリをまたいだ順序は定義されていません。

ただし`NULL`は例外で、いずれのカテゴリの値とも比較できます。`NULL`は他のどの値よりも小さく、`NULL`同士は等しいものとして扱われます。
