...
引数名の後に `()` を付けることで、その引数を **遅延評価引数** として宣言できます。

遅延評価引数は呼び出し時に即座に評価されず、その引数を受け取った変数は「引数を評価する関数」になります。
関数本体内で `引数名()` と呼び出した時点で、はじめて評価が行われます。
...
次の例では、 `cond` の値に応じて `then` と `else` のどちらか一方だけが評価されます。
