...
`label !!` の右辺を省略すると、 `NULL` を返しつつ即座にそのラベルを脱出できます。
...
文法上、 `!!` の直後には空白・改行を許容する `b` を挟んだうえで右辺を optional な `commas` で受けているため、右辺が書かれていれば従来通りその式が評価され、右辺が無ければ `NULL` が返ります。
そのため、 `!!` の直後で改行していても、次の行に式があればこれまで通りそれが右辺として結合されます。
...
一方で、これまで構文エラーになっていた `label !!` 単体の記述は正常にパースされるようになり、以降の行が実行される前にラベルから脱出する挙動になる点に注意してください。
...
リターン演算子は特殊な前置演算子として扱われており、 `value` 節にはラベル系演算子よりも結合優先度の高い任意の式が来れます。
...
ラベル演算子はラベル系演算子に属し、カンマ `,` よりも低く、パイプ系 `|` よりも高い結合優先度を持ちます。
...
スロー演算子もリターン演算子 `label !! value` と同様の結合規則を持ち、右辺にはラベル系演算子よりも結合優先度の高い任意の式が来れます。
...
トライキャッチ演算子はラベル系演算子に属し、カンマ `,` よりも低く、パイプ系 `|` よりも高い結合優先度を持ちます。
