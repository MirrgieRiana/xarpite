...
`label !!` の右辺を省略すると `NULL` を返しつつ即座に脱出します。右辺を結合したい場合は `label !! 123` のように同一行に書きます。 `!!` の直後で改行すると右辺は省略扱いになり、以降の行は実行されません。

また、 `!!` の直後に `!?` を続けても右辺には取り込まれず、外側のトライキャッチ演算子として解釈されます。たとえば `label !! !? "On Error!"` は `(label !!) !? "On Error!"` と等価です。
